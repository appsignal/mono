...
RSpec.describe Mono::Cli::Publish do
  ...
  context "without CHANGELOG" do
    it "commits the CHANGELOG" do
      ...
      next_version = "1.2.4"
      tag = "v#{next_version}"
      tmp_changelog_file = tmp_changelog_file_for(current_project)
  ...
  context "Git tag" do
    it "adds the changelog contents to the tag" do
      prepare_ruby_project do
        create_ruby_package_files :name => "mygem", :version => "1.2.3"
        create_changelog
        add_changeset(:minor, :type => "add")
  ...
  context "with only one package selected in a mono repo" do
    it "publishes only the selected package" do
      ...
      tmp_changelog_file = tmp_changelog_file_for("package_a")
  ...
  context "with multiple packages selected in a mono repo" do
    it "publishes only the selected packages" do
      ...
      package_a_changelog_file = tmp_changelog_file_for("package_a")
      package_b_changelog_file = tmp_changelog_file_for("package_b")
      ...
      expect(performed_commands).to eql([
        ...
        [project_dir, version_tag_command(tag1, package_a_changelog_file)],
  ...
  context "with --yes" do
    it "doesn't prompt for confirmation" do
      prepare_ruby_project do
        create_ruby_package_files :name => "mygem", :version => "1.2.3"
        add_changeset :patch
      end
      output = run_publish(["--yes"], :lang => :ruby)
